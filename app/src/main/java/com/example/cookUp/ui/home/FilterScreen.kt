...
import com.example.cookUp.ui.components.FilterChip
...
fun FilterScreen(
    ...
    var sortState by remember { mutableStateOf(RecipeRepo.getSortDefault()) }
    var maxCalories by remember { mutableFloatStateOf(0f) }
    val defaultFilter = RecipeRepo.getSortDefault()

    Dialog(onDismissRequest = onDismiss) {

        val categoryFilters = remember { RecipeRepo.getCategoryFilters() }
        val lifeStyleFilters = remember { RecipeRepo.getLifeStyleFilters() }
        CookUpScaffold(
            topBar = {
                TopAppBar(
                    navigationIcon = {
                    ...
                    backgroundColor = CookUpTheme.colors.uiBackground
                )
            }
        ) {
            Column(
...
@Composable
fun SortFilters(
    sortFilters: List<Filter> = RecipeRepo.getSortFilters(),
    sortState: String,
...
fun MaxCalories(sliderPosition: Float, onValueChanged: (Float) -> Unit) {
    ...
    Slider(
        ...
        colors = SliderDefaults.colors(
            thumbColor = CookUpTheme.colors.brand,
            activeTrackColor = CookUpTheme.colors.brand
        )
    )
}
